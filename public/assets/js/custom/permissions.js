$(document).ready(function() {
  ...
  $(".student-select2").on("change", function() {          
    $(".schedule-select2").html("")
    $(".schedule-select2").append(`<option value="">Pilih Jam</option>`)    
      
    $(".end-schedule-select2").html("")
    $(".end-schedule-select2").append(`<option value="">Pilih Jam</option>`)
    $.ajax({
      type: "GET",
      url: `/students/${$(this).val()}/json`,
      dataType: "json",
      success: function({student}){        
        ...
        schedule_details.forEach(schedule => {            
          $(".schedule-select2").append(`
            <option value="${schedule.id}">
              ${formatTime(schedule.start_time)} - ${formatTime(schedule.end_time)} (${schedule.subject_teacher.subject.name})
            </option>
          `)        
        });
  ...
  $.ajax({
    type: "GET",
    url: `/students/${$(".student_id").val()}/json`,
    dataType: "json",
    success: function({student}){     
      const date = new Date()
      const schedule_today = student.group.schedules.filter((schedule) => schedule.day == date.getDay())[0]
