    ...
    <div class="table-wrapper mt-[20px] input-teacher">
        <form action="{{ route('facilities.store') }}" method="post" enctype="multipart/form-data"
            ...
            <label for="name" class="form-label required">Nama Lokasi</label>
                <input type="text" id="name" name="name" placeholder="Nama lokasi sarana dan prasarana..." class="input-crud" value="{{ old('name') }}" required />
                @error('name')
            ...
            <div class="col-span-12 md:col-span-6 flex flex-col mt-2.5">
                ...
                <input type="text" id="description" name="description" placeholder="Deskripsi lokasi sarana dan prasarana..." class="input-crud" value="{{ old('description') }}" />
                @error('description')
                    <div class="text-danger mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-span-12 sm:col-span-4 flex flex-col">
                ...
                <label for="path" class="d-block mb-3">
                    <img src="{{ asset('assets/img/upload-image.jpg') }}" class="create-facility-preview-img border"
                        width="400" alt="">
                </label>
                <input type="file" id="path" name="path"
                    class="input-crud py-0 create-facility-input" />
                @error('path')
                    <div class="text-danger mt-1">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-span-12 flex items-center gap-3 mt-2">
                ...
                <a href="{{ route('facilities.index') }}" class="button btn-second text-white">Batal Tambah</a>
            </div>
        </form>
    </div>
@endsection
